...
app.get('/api/logs', (req, res) => {
  ...
  exec(`tail -n 50 "${logFile}"`, (error, stdout, stderr) => {
    if (error) {
      res.json({
        success: false,
        logs: ['No log file found or unable to read logs'],
    ...
    res.json({
      success: true,
      logs: logs.length > 0 ? logs : ['No logs found'],
      error: null,
      note: 'Showing last 50 log entries'
    });
  });
});
...
app.post('/api/test-connection', (req, res) => {
  res.json({
    success: false,
    message: 'SMB testing is disabled in web interface. Please configure SMB settings in .env file and restart services.',
    tests: []
  });
  return;
